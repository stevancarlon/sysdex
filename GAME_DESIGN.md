...
The current Phase 2 implementation runs the inherited graph and every player-authored alternative through the shared evaluator. It exposes synchronous versus asynchronous routes, event delivery, backlog, and freshness without checking for a Message Queue recipe; deeper retry, duplication, and ordering behavior remains part of the discrete-event migration.
...
Implementation checkpoint: all eight campaign operations now run through the UI-independent directed-graph evaluator in `src/simulation/`. Request capacity only credits machines on a complete response path; caches accelerate only paths that traverse them; replicated standbys remain operationally meaningful without pretending to serve reads; and synchronous calls, asynchronous queues, consumers, commits, failures, event backlog, and delivery lag come from edge semantics. Matching requires a bounded Geo Index query route. Dispatch evaluates its request route and location-event route separately. Streaming independently evaluates metadata, playback delivery, and transcoding, so a healthy API cannot conceal a broken CDN or media pipeline. Analytics, transcode, and driver-location workloads now declare visible delivery and freshness contracts; a shared correctness evaluator derives missing, backlogged, stale, blocking, or healthy states from graph arrival, drain, and lag signals, so “fast but wrong” designs cannot certify. Every post-tutorial phase offers optional manual wiring with directed compatible ports and recoverable auto-routing. The live graph inspector follows the same directed connections to visualize blocking request paths, playback delivery, and asynchronous event paths—including player-authored cables. Later incidents no longer demand a prescribed purchase: existing redundancy may absorb the failure, or any player-authored topology change that restores baseline throughput and required background delivery can begin recovery; final certification still requires the full SLO. Versioned phase-local blueprints preserve machines, runbook policies, automatic/manual mode, and authored cables, allowing reversible experimentation. The same designs can now be encoded into compact URL-safe tuples and shared; imported links pass the full phase, unlock, grid, ID, budget, cable-compatibility, duplicate-edge, and label validator before any state is restored. Free Lab provides a separate creative-mode runtime for all five workload families: players choose a 500–10,000 r/s peak, author their own p95 and error targets plus delivery/freshness contracts where relevant, and run reproducible steady, wave, or burst traces selected by replay seed. Every component and runbook is available, traffic runs continuously, and no scripted incident, certification modal, or campaign progression side effect exists. Players can take any selected machine offline and restore it while traffic runs; broken graphs remain runnable for observation, and trace parameters plus injected failure state survive stopping, isolated local saves, and validated shared URLs. Every completed campaign drill produces a graph-aware operational review: what the run proved, what risk remains, and a comparative experiment to try next without declaring a canonical topology. Versioned phase-local drill snapshots compare consecutive experiments across capacity, p95, spend, machine count, routing mode, and background behavior, turning topology iteration into measurable A/B learning. Discrete per-request flow, duplicate/order semantics, emergent failure generation, and fully community-authored workload rules remain the next migration layers.
